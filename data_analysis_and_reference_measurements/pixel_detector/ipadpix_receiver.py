...
parser = argparse.ArgumentParser(description="Receive iPadPix UDP packets")
parser.add_argument("--host", default="0.0.0.0", help="Local interface/IP to bind")
parser.add_argument("--port", type=int, default=8123)
parser.add_argument("--output-dir", default="data", help="Directory for trusted legacy .pkl output")
args = parser.parse_args()
HOSTNAME, UDP_PORT = args.host, args.port
...
schema = avro.schema.Parse((Path(__file__).with_name("ipadpix_schema.json")).read_text())
...
while True:
    try:
        ...
        for cluster in records['clusterArray']:
            ...
            df_new = df_new.assign(ts=datetime.datetime.fromtimestamp(timestamp))
        ...
        if creation_time:
            ...
            df.to_pickle(Path(args.output_dir) / (creation_time.strftime("%Y-%m-%d_%H:%M:%S") + "___" + str(counter) + "___" + td_str + ".pkl"))
        else:
