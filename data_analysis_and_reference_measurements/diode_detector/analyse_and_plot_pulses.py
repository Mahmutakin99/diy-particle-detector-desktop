...
parser = argparse.ArgumentParser(description="Analyse a .pdet, .msgp, or explicitly trusted legacy .pkl recording")
parser.add_argument("recording", type=Path)
parser.add_argument("--trusted-pickle", action="store_true", help="Permit decoding a trusted local pickle file")
args = parser.parse_args()
recording = load_recording(args.recording, trusted_pickle=args.trusted_pickle)
df = pd.DataFrame({"ts": pd.to_datetime([p.timestamp_us for p in recording.pulses], unit="us", utc=True), "pulse": [np.asarray(p.waveform, dtype="int16") for p in recording.pulses]})
if df.empty:
    raise SystemExit("The recording contains no pulses")
...
if not peaks:
    raise SystemExit("No valid pulses matched the current threshold and waveform filters.")
...
xpulses_n=np.array([np.append(xi,[0]*(length-len(xi))) for xi in xpulses], dtype=np.int64)
length = max(map(len, ypulses))
ypulses_n=np.array([np.append(yi,[0]*(length-len(yi))) for yi in ypulses], dtype=np.int64)
