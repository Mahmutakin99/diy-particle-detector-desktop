...
## Modern recording application / Güncel kayıt uygulaması

`particle_detector` provides the maintained desktop recorder and a matching web recorder. It supports both detector profiles: electron/beta and alpha spectrometer. New recordings are written outside the repository in the platform application-data directory and use the portable, versioned `.pdet` MessagePack format.
...
The desktop application follows the system language (Turkish or English) and can change language and appearance at runtime. It keeps energy axes unavailable until the user enters at least two calibration reference points. A raw amplitude must not be interpreted as dose, isotope, keV, or MeV.

### File compatibility / Dosya uyumluluğu

- `.pdet`: portable session header, detector profile, sampling rate, timestamped pulses, data-loss count, and optional user calibration.
- Legacy `.msgp`: importable from the former browser recorder.
- Legacy `.pkl`: importable only after an explicit trusted-local-file acknowledgement because pickle can execute code during import. Both historical DataFrame layouts are handled.

The iPadPix tools remain independent command-line tools. `ipadpix_receiver.py --host 0.0.0.0 --port 8123 --output-dir data` configures reception; `plot_pixel_data.py --input FILE --second-input FILE` configures plotting inputs.
...
Türkçe: Apple Silicon Mac için `macos-arm64.dmg`, Intel Mac için `macos-x64.dmg` dosyasını indirin; içindeki uygulamayı **Uygulamalar** klasörüne sürükleyip açın. Linux'ta işlemciye uygun AppImage dosyasına çalıştırma izni verip açın. Windows'ta işlemciye uygun `windows-*-Setup.exe` yükleyicisini çalıştırın ve uygulamayı Başlat menüsünden açın. Ses girişi iznini işletim sistemi istediğinde verin; gerçek dedektör ölçümü için ayrıca donanım testi gerekir.
